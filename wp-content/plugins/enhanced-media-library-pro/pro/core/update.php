...
if ( ! function_exists( 'wpuxss_eml_pro_update_transient' ) ) {
    ...
    function wpuxss_eml_pro_update_transient( $transient, $license_key ) {
        ...
        if ( is_wp_error( $new_version ) || version_compare( $new_version, $wpuxss_eml_version, '<=' ) ) {
            return $transient;
        }
        ...
        $info->package = ! is_wp_error( wpuxss_eml_pro_remote_info( 'is-license-active', $license_key ) ) ? wpuxss_eml_pro_get_url() . '?' . build_query( $args ) : '';
...
if ( ! function_exists( 'wpuxss_eml_pro_plugins_api' ) ) {
    ...
    function wpuxss_eml_pro_plugins_api( $res, $action, $args ) {
        ...
        if ( ! is_wp_error( $body ) ) {
            ...
            $res->download_link = ! is_wp_error( wpuxss_eml_pro_remote_info( 'is-license-active', $license_key ) ) ? wpuxss_eml_pro_get_url() . '?' . build_query( $wpuxss_eml_args ) : '';
...
if ( ! function_exists( 'wpuxss_eml_pro_in_plugin_update_message' ) ) {
    ...
    function wpuxss_eml_pro_in_plugin_update_message( $plugin_data, $r ) {
        ...
        $license_key = get_option( 'wpuxss_eml_pro_license_key', '' );
        $is_license_active = wpuxss_eml_pro_remote_info( 'is-license-active', $license_key );

        if ( ! is_wp_error( $is_license_active ) && $is_license_active )
            return;
