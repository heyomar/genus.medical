		...
		$.ajax({
			...
			success: function (data) {
				$.each(data, function (i, val) {
					if (val['parent'] === 4) {
						$('#categoriesRow').append('<div class="card col-xs-12 col-sm-4"><div class="med-sm"><div class="image"><a href="/' + val['name'].replace(/\s+/g, '-').toLowerCase() + '"><span class="label">' + val['name'] +
						'</span<</a></div></div></div>')
